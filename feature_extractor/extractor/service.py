...
from common.constants import UPLOAD_CHUNK_BYTES, ExtractorKeys
from feature_extractor.extractor.base import FeatureExtractor
from feature_extractor.schemas import (
    PreparedImage, ExtractedFeatures, ExtractFeaturesRequest, ArrayPayload, FeatureMapPayload, ImageInfo, ModelInfo,
    ExtractFeaturesResponse
)
from feature_extractor.utils import convert_to_storage_dtype, encode_array_to_base64
...
async def extract_features_from_image(
    ...
    request: ExtractFeaturesRequest, extractor: FeatureExtractor
) -> ExtractFeaturesResponse:
    """Run the model on an already prepared image and build the response."""
    try:
        ...
        response = _build_feature_response(
            filename, content_type, prepared, result, extractor.info, request.include_feature_map
        )

        logger.info(
            f"Extracted features from {filename} ({prepared.original_hw[1]}x{prepared.original_hw[0]}): "
            f"vector {tuple(result.feature_vector.shape)}, {len(response.feature_maps)} feature maps, "
